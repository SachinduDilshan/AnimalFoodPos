...
function focusField(container, itemId, field) {
  const el = container?.querySelector(`[data-row-id="${itemId}"] [data-field="${field}"]`)
  el?.focus()
  el?.select?.()
}

export function CartTable({ lines, onUpdateLine, onRemoveLine, focusRequest, onFocusSearch }) {
  ...
  useEffect(() => {
    ...
    if (line.discountType === 'NONE') {
      ...
      focusField(containerRef.current, pendingDiscountFocus.itemId, 'discount-value')
    }
              ...
              <TableRow key={line.itemId} data-row-id={line.itemId}>
                ...
                <TableCell>
                  <Input
                    ...
                    onKeyDown={(e) => handleFieldKeyDown(e, line.itemId, 'rate')}
                  />
                </TableCell>
                <TableCell>
                  <Input
                    ...
                    onKeyDown={(e) => handleFieldKeyDown(e, line.itemId, 'discount-type')}
                  />
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Select
                      ...
                      onValueChange={(value) => {
                        onUpdateLine(line.itemId, { discountType: value, discountValue: 0 })
                        setPendingDiscountFocus({ itemId: line.itemId })
                      }}
                    >
                      <SelectTrigger className="w-24" data-field="discount-type">
                        <SelectValue />
                      </SelectTrigger>
                      {/*
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => handleFieldKeyDown(e, line.itemId, null)}
                    />
                  </div>
                </TableCell>
